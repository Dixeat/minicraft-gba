...
static void pause_draw(void) {
    ...
    if(should_save) {
        ...
        screen_write(text(TEXT_OVERWRITE_FILE), 6, pause_x + 1, pause_y + 4);

        WRITE_ANSWER(TEXT_YES, 0, pause_x + 4,  pause_y + 6);
        WRITE_ANSWER(TEXT_NO, 1, pause_x + 12, pause_y + 6);
    } else {
        WRITE_MENU_OPTION(TEXT_SAVE_GAME, SAVE_OPTION, pause_x + 2, pause_y + 4);
        WRITE_MENU_OPTION(TEXT_QUIT, QUIT_OPTION, pause_x + 4, pause_y + 6);
    }
}
